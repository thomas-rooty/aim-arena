...
export const useKeyPress = (target: string[], event: (pressed: boolean) => void): void => {
  useEffect(() => {
    const downHandler = ({key}: { key: any }) => target.indexOf(key) !== -1 && event(true);
    const upHandler = ({key}: { key: any }) => target.indexOf(key) !== -1 && event(false);
